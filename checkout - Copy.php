...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['place-order-button'])) {
    ...
    $pictureFilename = '';
    if ($_FILES['picture-image']['error'] === UPLOAD_ERR_OK) {
      $image_folder = 'uploaded_img/';
      $pictureFilename = $_FILES['picture-image']['name'];
      $pictureTmpName = $_FILES['picture-image']['tmp_name'];
      $uploadPath = $image_folder . $pictureFilename;

      // Move the uploaded file to the directory
      if (move_uploaded_file($pictureTmpName, $uploadPath)) {
        // File uploaded successfully

        // Store the file path in the database
        $insertPictureQuery = $conn->prepare("UPDATE tb_orders SET picture_image = ? WHERE user_id = ?");
        $insertPictureQuery->execute([$uploadPath, $user_id]);
        ...
        $errors[] = 'Failed to upload the ID picture.';
      }
    }
  }
}
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const selectedDeliveryInput = document.getElementById("selected-delivery");
    const selectedPaymentInput = document.getElementById("selected-payment");

    deliveryOptions.forEach(option => {
      option.addEventListener("click", function() {
        const selectedValue = this.getAttribute("data-value");
        selectedDeliveryInput.value = selectedValue;
        this.closest(".dropdown").querySelector(".dropbtn").textContent = selectedValue;
      });
    });
    ...
    paymentOptions.forEach(option => {
      option.addEventListener("click", function() {
        const selectedValue = this.getAttribute("data-value");
        selectedPaymentInput.value = selectedValue;
        this.closest(".dropdown").querySelector(".dropbtn").textContent = selectedValue;
      });
    });
  });
...
<body>
  ...
  <div class="display-orders">
    ...
    if (isset($_POST['proceed_to_checkout'])) {
      ...
      $cart_items[] = $name . ' (' . $price . ' x ' . $quantity . ') - ';
      $total_products = implode($cart_items);
    }
    ?>
    <div class="product-item">
      <img src="uploaded_img/<?= $image ?>" alt="" class="product-image">
      <div class="product-details">
        <form action="order-receipt.php" method="POST" enctype="multipart/form-data">
          <input type="hidden" name="pid" value="<?= $pid; ?>">
          <input type="hidden" name="name" value="<?= $name; ?>">
          <input type="hidden" name="price" value="<?= $price; ?>">
          <input type="hidden" name="quantity" value="<?= $quantity; ?>">
          <input type="hidden" name="image01" value="<?= $image; ?>">
          <span class="product-name"><?= $name ?> <span>(<?= '₱' . number_format($price, 2) . '/- x ' . $quantity ?>)</span></span>
          <p><span>Subtotal: <?= '₱' . number_format($price * $quantity, 2); ?></span></p>
      </div>
    </div>
